...
<h4>Colors Recently used:</h4>
<?

$sql = "SELECT id,tagcolor,title FROM calendarevents GROUP BY(tagcolor) Order By id DESC Limit 20"; 
$result = mysqli_query($con,$sql);
while($row = mysqli_fetch_array($result,MYSQLI_ASSOC))
{
$calendareventtitle = $row['title'];
...
echo"
<div style=\"width:80px; height:40px; background-color:$calendareventtagcolor; display:inline-block; overflow:hidden; padding:2px; cursor:pointer;\" title=\"$calendareventtitle\" onclick='ChoosePreviousColor(\"$calendareventtagcolor\")'>
$calendareventtitle
</div>
";
}
